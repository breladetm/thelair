    ...
    Schema.findOne({ Guild: interaction.guild.id, Code: id }, async (err, data) => {
        if (data) {
            Schema.findOneAndDelete({ Guild: interaction.guild.id, Code: id }).then(() => {
                client.succNormal({ text: `Entry **#${id}** has been deleted.`, type: 'editreply' }, interaction);
            })
        }
        else {
            client.errNormal({ error: `No entry found with the id **#${id}**`, type: 'editreply' }, interaction);
        }
    })
}
