...
module.exports = async (client) => {
    const startLogs = new Discord.WebhookClient({
        id: "1272652966230360159",
        token: "pYWa9nKhmAXIXnY8gxCcKDf40Dv4HVDT8QrsJlo4HdmdqgkgGzzmOUQj-KNZ98cyDWp4",
    });

    console.log(`\u001b[0m`);
    console.log(chalk.blue(chalk.bold(`The Lair`)), (chalk.white(`>>`)), chalk.green(`Started on`), chalk.red(`${client.guilds.cache.size}`), chalk.green(`servers!`))
    ...
    setInterval(async function () {
        ...
        if (process.env.DISCORD_STATUS) {
            ...
            statuttext = [
                `Why are you stalking my status?`
            ];
        }
        const randomText = statuttext[Math.floor(Math.random() * statuttext.length)];
        client.user.setPresence({ activities: [{ name: randomText, type: Discord.ActivityType.Custom }], status: 'online' });
